...
在 Keithley 控制的偏置（电压或电流）与 AWG 输出幅度（Vpp）两个维度上自动进行
二维扫描。每个网格点运行一次 DMT 流水线，Step 2 / Step 4 可重复测量并取平均，
结果保存为 CSV 汇总表以及兼容 CodePlot 的等高线图。
...
各次单独的 DMT 运行仍以其自身的 run_id 记录在
`data/records/` 和 `data/codeplot_assets/<point_run_id>/` 中，因此原始波形得以保留，
但默认不会在网格扫描 GUI 中显示。
"""
import json
import logging
...
class GridScanConfig:
    ...
    vpp_step: float = 0.1
    # 流水线选项
    run_mode: str = "step1-4"           # 取 "step1-4" 或 "step1-2"
    step4_repeats: int = 1              # 最终测量步骤的重复次数
    use_awg: bool = False
    use_nn: bool = False
    ...
    def __post_init__(self):
        ...
        if self.run_mode not in ("step1-4", "step1-2"):
            raise ValueError("run_mode 必须为 'step1-4' 或 'step1-2'")
        if self.step4_repeats < 1:
            raise ValueError("step4_repeats 必须 >= 1")
...
class GridScanner:
    ...
    def _run_point_step1_2(self, point_run_id: str):
        """运行 step1 + step2，将 step2 重复 N 次，返回平均指标。"""
        # 延迟导入，以便未安装 PyVISA 时 GUI 仍能打开
    ...
    def _run_point_step1_4(self, point_run_id: str):
        """运行 step1-4，将 step4 重复 N 次，返回平均指标。"""
        # 延迟导入，以便未安装 PyVISA 时 GUI 仍能打开
        from main import (
    ...
    def _run_point(self, param1: float, vpp: float, point_idx: int) -> Dict:
        ...
        self.logger.info(f"=== 点 {point_idx}：{self.cfg.param1_name}={param1}，Vpp={vpp} ===")
    ...
    def run(self, progress_callback=None) -> Path:
        ...
        self.logger.info(f"开始网格扫描 {self.cfg.scan_id}")
        ...
        self.logger.info(f"网格规模：{len(p1_grid)} x {len(vpp_grid)} = 共 {total} 个点")
