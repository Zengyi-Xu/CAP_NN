...
def step4_receive_bitloading(tx_dict: dict,
    ...
    if config.PLOT_SHOW:
        ...
        plot_snrs(load_txt(config.FINAL_SNR_QPSK),
                  res["SNR_R"],
                  f"Estimated vs Recovered SNR ({run_id})",
                  run_id, "SNR_compare")
        flat_iq = res["out2"].reshape(-1)
        plot_constellation(flat_iq,
                           f"Bitloading RX Constellation ({run_id})",
                           run_id, "bitloading_constellation")
